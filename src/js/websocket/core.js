...
GameScreenFactory.prototype.webSocketConnect = function(){
	console.log("webSocketConnect", {event: "connecting", result: "call"});
	
	this.isConnected = false;
	this.isWebSocketHaveError = false;
	this.webSocketInstance = new WebSocket(
		"wss://" + this.serverURL + ":" + this.serverPort
		+ "/" + this.serverEndpoint
	); 
...
GameScreenFactory.prototype.webSocketOnClose = function(event){
	console.warn("webSocketOnClose", event, {result: 'closed'});
	this.isConnected = false;
...
GameScreenFactory.prototype.webSocketOnOpen = function(event){
	console.log("webSocketOnOpen", event, {result: 'connected'});
...
GameScreenFactory.prototype.initGame = function(){
	console.log("webSocketOnMessage", event, {result: 'gamescreen_is_asked_to_init'});
...
GameScreenFactory.prototype.webSocketOnMessage = function(event){
	console.log("webSocketOnMessage", event, {result: 'message_received'});
	...
	try {
		...
		if 	(
			...
			console.log("webSocketOnMessage", this.gameInformation, {result: 'gameinformation_updated'});
		...
		if 	(
			...
			console.log("webSocketOnMessage", event, {result: 'gamescreen_need_to_update_data'});
			this.gameInformation.player1_name = networkData.player1_name;
			this.gameInformation.player2_name = networkData.player2_name;
			this.gameInformation.players_teamNumber = networkData.players_teamNumber;
		...
		if 	(
			...
			console.log("webSocketOnMessage", event, {result: 'gamescreen_need_to_display_camera'});
		...
		if 	(
			...
			console.log("webSocketOnMessage", event, {result: 'gamescreen_need_to_capture_a_photo'});
		...
		if 	(
			...
			console.log("webSocketOnMessage", event, {result: 'gamescreen_is_asked_to_launch_game'});
...
GameScreenFactory.prototype.webSocketOnError = function(event){
	console.warn("webSocketOnError", event, {result: 'error_occured'});
...
GameScreenFactory.prototype.webSocketSend = function(data){
	if (this.isConnected) {
		try {
			...
			console.log("webSocketSend", networkData, {result: 'success'});
		} catch(e) {
			console.warn("webSocketSend", data, {result: 'parse failed'});
		}
	} else {
		console.warn("webSocketSend", data, {result: 'network failed'});
	}
};

//Update the current level on the game server
GameScreenFactory.prototype.gameInformationLevel = function(level){
	console.log("gameInformationLevel", null, {result: 'call'});
	this.gameInformation.players_teamLevel = parseInt(level);

	console.log("gameInformationLevel", this.gameInformation, {result: 'sent'});
	this.webSocketSend(this.gameInformation);
};


GameScreenFactory.prototype.gameInformationTutorialEnded = function(){
	console.log("gameInformationTutorialEnded", null, {result: 'call'});
	...
	console.log("gameInformationTutorialEnded", data, {result: 'sent'});
	this.webSocketSend(data);
...
GameScreenFactory.prototype.cameraFeedTakePicture = function(){
	console.log("CameraFeedTakePicture", null, {result: 'call'});

	var players_souvenir = this.takePicture();

	console.log("CameraFeedTakePicture", players_souvenir, {result: 'sent'});
...
GameScreenFactory.prototype.takePicture = function(){
	console.log("TakePicture", null, {result: 'call'});
	...
	try {
  		Webcam.snap( function(data_uri) {
			...
			console.log("TakePicture", null, {result: 'success'});
        } );
	} catch (e) {
